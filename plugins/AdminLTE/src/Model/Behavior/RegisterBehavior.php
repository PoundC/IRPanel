...
class RegisterBehavior extends BaseTokenBehavior
{
    ...
    {
        $explodedFullName = explode(' ', $data['fullname']);

        if(count($explodedFullName) > 0) {

            $user->set('first_name', $explodedFullName[0]);
        }

        if(count($explodedFullName) > 1) {

            array_shift($explodedFullName);

            $last_name = '';
            foreach($explodedFullName as $name) {
                $last_name .= $name . ' ';
            }
