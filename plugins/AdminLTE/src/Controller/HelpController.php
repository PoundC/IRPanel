...
class HelpController extends AppController
{
    ...
    public function help($id = 0)
    {
        if($this->request->getMethod() == 'POST') {
            ...
            $questionsQuery = $this->FaqQuestions->find('all', ['contain' => ['FaqAnswers', 'FaqAnswers.FaqTopics']])
                                             ...
                                             ->orWhere(['FaqAnswers.subject LIKE' => '%' . $data['search'] . '%']);
            ...
            if($id == '0') {
                ...
                $this->loadModel('AdminLTE.FaqTopics');

                $topicsQuery = $this->FaqTopics->find('all', ['contain' => ['FaqAnswers']]);
    ...
    {
        $answerTable = TableRegistry::get('AdminLTE.faq_answers');
        $tagsTable = TableRegistry::get('AdminLTE.faq_tags');
        $tagAnswersTable = TableRegistry::get('AdminLTE.faq_answer_tags');
        ...
        if($this->request->getMethod() == 'POST') {
            ...
            $answerTagsTable = TableRegistry::get('AdminLTE.faq_answer_tags');
            ...
            $answerQuestionsTable = TableRegistry::get('AdminLTE.faq_questions');
    ...
    {
        $chatRoomsTable = TableRegistry::get('AdminLTE.chat_chatrooms');
        ...
        $helpTabsTable = TableRegistry::get('AdminLTE.chat_helptabs');
